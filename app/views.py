...
def _paper_size(request):
    ...
    return value if value in {"A4","SHORT"} else "A4"
...
def print_sheet(request,quiz_id):
    quiz=get_object_or_404(Quiz.objects.prefetch_related("questions"),pk=quiz_id)
    return render(request,"app/print_sheet.html",{"quiz":quiz,"paper_size":_paper_size(request)})

def camera_scan(request,quiz_id):
    quiz=get_object_or_404(Quiz.objects.prefetch_related("questions"),pk=quiz_id)
    return render(request,"app/camera_scan.html",{"quiz":quiz,"paper_size":_paper_size(request)})
...
def camera_submit(request,quiz_id):
    ...
    if not isinstance(image,UploadedFile): return JsonResponse({"ok":False,"error":"No answer-sheet image was uploaded."},status=400)
    if image.size>15*1024*1024: return JsonResponse({"ok":False,"error":"Image is too large. Maximum is 15 MB."},status=400)
    ...
    try: result=scan_answer_sheet(submission.answer_sheet.path,quiz,paper_size)
        ...
        submission.status=Submission.STATUS_ERROR; submission.scan_message="The scanner encountered an unexpected processing error."; submission.save(update_fields=["status","scan_message"])
